...
- **前端筛选**：路径搜索框、「隐藏机器人」（**默认勾选**）、「仅有效访问（有停留）」复选框；数据已内联（最多 1000 条），且统计卡/趋势/交叉/明细均随筛选实时联动，默认即隐藏机器人、聚焦真实读者。
- **探测路径拦截**：中间件对恶意/扫描路径（`/.git`、`/.env`、`/robots.txt`、`/sitemap.xml`、`/wp-admin*`、`/server.key`、`/dump.sql`、`.php/.sql/.bak` 等，以及 3 段及以上深路径）返回 `404` 且**不写入访问日志**，从源头避免安全暴露与统计污染（详见 7.2）。
